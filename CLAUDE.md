...
| 3日分の在庫を20 goroutine が取り合う（半数は**逆順**）→ デッドロックが発生しない | **通過** | 同 `TestNoDeadlockAcrossDates` |

並行テストは `Transactor` + `Repository` + 集約だけで書きます。`Service` は使いません。
守られるべきはこの3つの組み合わせであって、`Service` の実装ではないからです。

デッドロックのテストで半数を**逆順**にしているのは意図的です。複数行を1トランザクションでロックする設計なら
必ずデッドロックする条件ですが、1トランザクション1集約では1つのトランザクションが握るロックは常に1行なので、
順序に関係なく起きません。`StayPeriod.Dates()` の昇順は、将来トランザクションの粒度を変えたときの保険です。

これらのテストは CI（`.github/workflows/ci.yml`）で push ごとに実行します。
GitHub のランナーには Docker があるので testcontainers がそのまま動きます。
...
**補償が失敗しても致命的ではありません。** 期限切れ回収ワーカーが後で拾います。二重の安全網です。

現在の補償はアプリケーション層のメモリ上で行われ、プロセスマネージャーはまだありません（8章で導入）。

### 期限切れ回収

`cmd/worker` が一定間隔（既定1分、`WORKER_INTERVAL`）で2つの処理を順に走らせます。

1. **予約側**：作成から30分（`HoldTTL`）を過ぎた仮予約を `Expired` 理由でキャンセルし、在庫を解放する
   （`booking/app.ExpireStale`）。通常の経路と同じ `Cancel` を通るので、キャンセル料は0円で確定する
2. **在庫側**：期限切れの仮確保を回収する（`inventory/app.CollectExpired`）

予約側を先に走らせるのは、予約と在庫の両方を整合した状態で片付けられるからです。
在庫側は、予約側が何らかの理由で取りこぼした確保を拾う安全網です。
在庫側で先に確保が消えていた場合、予約側の解放は `ErrHoldAlreadyReleased` として扱い、失敗にしません。

どちらも1件ずつ別トランザクションで処理し、何度走らせても結果は変わりません（冪等）。
`make collect` で1回だけ実行できます。

決済処理中（`processing_payment`）で止まった予約はこのワーカーの対象外です。
課金が成立している可能性があるため、8章の照合処理が決済サービスに問い合わせて決着させます。
...
| `timeout` | 何もしない。予約も在庫もそのまま残し、504 を返す。30分後に期限切れ回収ワーカーが取り消す |
...
段階1〜6（Repository → 並行テスト → 検索 → アプリケーション層 → API → フロント）と
期限切れ回収ワーカーは完了しました。以降の順序です。
...
│   │   ├── seed/         # 動作確認用のデモデータ
│   │   └── worker/       # 期限切れ回収（決済照合は未着手）。-once で1回だけ実行
│   ├── internal/
